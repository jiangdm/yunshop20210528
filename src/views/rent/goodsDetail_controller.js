  ...
  methods:
    {
        ...
        $http.get("plugin.lease.frontend.modules.goods.controllers.goods.get-goods", json, "加载中...").then(function(response) {
          ...
          if (response.result == 1) {
            ...
            that.fun.wxShare(
              ...
              {
                title: (that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_title)) ? that.goodsInfo.title : that.goodsInfo.has_one_share.share_title,
                imgUrl: (that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_thumb)) ? that.goodsInfo.thumb : that.goodsInfo.has_one_share.share_thumb,
                description: (that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_desc)) ? "" : that.goodsInfo.has_one_share.share_desc
              }
            );
      ...
      addGoods() {
        ...
        if (specsManage.length == this.goodsInfo.has_many_specs.length && this.goodsCount == optionsMaxCount) {
          ...
          this.$dialog.alert({ message: "数量超出范围" });
          return;
        }
        this.goodsCount++;
      ...
      confirm() {
        ...
        if (!this.isGoods) {
          this.$dialog.alert({ message: "商品已下架或不存在" });
          return;
        }

        if (this.goodsInfo.has_option == 1 && this.fun.isTextEmpty(optionsId)) {
          this.$dialog.alert({ message: this.goodsDescription });
          return;
        }

        if (this.fun.isTextEmpty(this.shopdatasItem)) {
          this.$dialog.alert({ message: "请选择租赁周期" });
          return;
        }

        if (this.fun.isTextEmpty(this.rentDate)) {
          this.$dialog.alert({ message: "请选择租赁起始日期" });
          return;
        }
      ...
      buyNowRequest(_goodsId, _optionsId, _total, _rentDate, _rentDay, _time_unit) {
        this.$router.push(this.fun.getUrl("confirmOrder", {
          tag: submitActionTag,
          goodsId: _goodsId,
          optionsId: _optionsId,
          total: _total,
          rentDate: this.fun.getTimestamp(_rentDate),
          rentDay: _rentDay,
