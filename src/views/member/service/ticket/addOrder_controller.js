  ...
  methods: {
    ...
    refreshAirMans() {
      ...
      if (this.airMans.length > 0) {
        ...
        //同步数据
        this.popSelectAirMans = this.airMans.filter((element, index, array) => {
          return element.choose == true;
        });
        //重新计算count
    ...
    addAirMans() {
      if (this.popSelectAirMans.length > 5) {
        this.$dialog.alert({ message: "最多不能超过5位乘机人" });
    ...
    confirmAddPassenger() {
      ...
      if (this.fun.isTextEmpty(this.cardId)) {
        this.$dialog.alert({ message: "请填写证件号" });
    ...
    removePeople(item) {
      ...
      //更新选择数量
      this.count = (this.airMans.filter((element, index, array) => {
        return element.choose == true;
      })).length;
    ...
    submit() {
      if (this.tempSelectAirMans.length == 0) {
        this.$dialog.alert({ message: "请选择乘机人" });
        return;
      }

      if (this.fun.isTextEmpty(this.linkmanName)) {
        this.$dialog.alert({ message: "请填写联系人姓名" });

        return;
      }

      if (this.fun.isTextEmpty(this.linkmanPhone)) {
        this.$dialog.alert({ message: "请填写联系人手机号" });
