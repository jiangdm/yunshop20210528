...
export default {
  ...
  methods: {
    ...
    getData() {
      let that = this;
      $http.get('plugin.team-fjyx.api.index.getCode', {},"获取中").then(function (response) {
    ...
    verifyCode(value)
    {
      ...
      let that = this;
      let json ={code:value};
      $http.get('plugin.team-fjyx.api.index.useCode', json,"激活中").then(function (response) {
        if (response.result == 1) {
          Toast(response.msg);
          that.getData();//重新获取数据
        } else {
